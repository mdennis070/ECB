...
class move_validation:
    ...
    def piece_legal_moves2(self, board, piece):
        legal_moves_list = []
        ...
        for move in all_legal_moves:
            if move.from_square == piece:
                legal_moves_list.append(move.to_square)
            #
        #
    ...
    def determine_move_made(self, prev_board, white_pos, black_pos):
        ...
        for itr in range(1, 8, 2):
            white_pos[itr] = white_pos[itr][::-1]
            black_pos[itr] = black_pos[itr][::-1]
        ...
        #print("w to   ", len_w_to_here)
        #print("w from ", len_w_from_here)
        #print("b to   ", len_b_to_here)
        #print("b from ", len_b_from_here)
        ...
        if w_capture_made or b_capture_made:
            if current_turn == chess.WHITE:
                capture_list = chess.Move(w_moved_from_here[0], b_moved_to_here[0])
            else:
